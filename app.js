...
function updateHistory() {
    ...
    var historyHtml = analysisHistory.map(function(item) {
        ...
        return '<div class="p-3 bg-gray-50 rounded-lg border border-gray-200 hover:border-blue-300 transition-colors cursor-pointer">' +
            ...
            '<p class="text-sm font-medium text-gray-900 mb-1">' + item.category + '</p>' +
            '<p class="text-xs ' + confColor + ' font-semibold">Confidence: ' + item.confidence + '%</p>' +
        '</div>';
    }).join('');
...
function updateStats() {
    ...
    if (analysisCount) {
        analysisCount.textContent = analysisHistory.length;
    }
}
